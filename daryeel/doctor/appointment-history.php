...
if (strlen($_SESSION['id']) == 0) {
    ...
    if (isset($_GET['cancel']) && isset($_GET['id']) && is_numeric($_GET['id'])) {
        $stmt = mysqli_prepare($con, "UPDATE appointment SET doctorStatus='0' WHERE id = ?");
    ...
    if (isset($_GET['delete']) && isset($_GET['id']) && is_numeric($_GET['id'])) {
        $stmt = mysqli_prepare($con, "DELETE FROM appointment WHERE id = ?");
        mysqli_stmt_bind_param($stmt, "i", $_GET['id']);
        mysqli_stmt_execute($stmt);
...
<tr>
    ...
    <td>
        ...
        elseif ($row['userStatus'] == 1 && $row['doctorStatus'] == 0) echo "Cancelled by You";
        ?>
    </td>
    <td>
        <?php if ($row['userStatus'] == 1 && $row['doctorStatus'] == 1) { ?>
            <a href="appointment-history.php?id=<?php echo $row['id']; ?>&cancel=1"
               ...
               class="btn btn-warning btn-xs">Cancel</a>
        <?php } else { echo "Canceled"; } ?>

        <a href="appointment-history.php?id=<?php echo $row['id']; ?>&delete=1"
